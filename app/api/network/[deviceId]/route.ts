...
export async function GET(
  ...
  // Fetch device configs + members in parallel
  const deviceIds = extractDeviceIds(topology ?? networks);
  const [members, ...configResults] = await Promise.all([
    safeFetch(
  ...
  return NextResponse.json({ networks, topology, mesh, configs, members, rootDeviceId: deviceId });
}
