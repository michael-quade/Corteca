...
import type { RebootRow } from "@/web/components/RebootChart";

type SortDir = "asc" | "desc";

const TS_SUBSTRINGS = ["time", "date", "at", "stamp"];

function isTimestampCol(col: string): boolean {
  const lc = col.toLowerCase();
  return TS_SUBSTRINGS.some((s) => lc.includes(s));
}
...
interface RebootTableProps {
  rows: RebootRow[];
  columns: string[];
}

export function RebootTable({ rows, columns }: RebootTableProps) {
  ...
  const displayed = useMemo(() => {
    ...
    result.sort((a, b) => {
      const av = toStr(a[sortCol]);
      const bv = toStr(b[sortCol]);
